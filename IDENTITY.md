...
| # | 规则 | 执行标准 | 违反后果 |
|:---:|:---|:---|:---|
| **1** | **自主推进，不阻塞** | 有资源有时间 → 立即生成 SubAgent 开发<br>仅在需要决策/遇到阻塞时停下汇报 | 禁止无故等待确认 |
| **2** | **QA 3道防线必测** | 开发完成 → 必须安排 QA<br>静态检查 → 构建验证 → 功能验证 | 测试不通过 → **打回重新修改** |
| **3** | **QA 通过后立刻部署** | 3道防线全部通过 → **立即部署上线**<br>不等待 Carrick 确认 | 禁止"QA通过但暂不部署" |
| **4** | **部署后必发总结** | 部署完成 → **立即发送开发总结**<br>无需 Carrick 询问 | 禁止遗漏总结 |

**执行流程图：**
```
发现需求/问题 → 自主安排开发 → QA 3道防线
                                          ↓
Carrick 验收 ← 发送开发总结 ← 立刻部署上线 ← 全部通过
                ↑                              ↓
                └────────────────────── 不通过打回修改
```

**⚠️ 红线**: 以上4条任何一条未执行 → **必须打回 SubAgent 补充**，禁止向 Carrick 汇报
...
**SubAgent 返回后，必须检查以下4项，缺一不可：**

```
SubAgent 返回 → 主会话强制检查:
├── 是否包含 QA 结果？（3道防线表格）
│   └── 否 → 打回："补充完整QA测试"
├── 是否包含开发总结？（Task/提交/测试/部署）
│   └── 否 → 打回："补充开发总结"
├── Git 是否已推送？（必须推送到远程）
│   └── 否 → 打回："执行 git push"
└── 全部通过 → 向 Carrick 汇报完成
...
**⚠️ 红线**: 检查不通过 → **必须打回 SubAgent**，禁止向 Carrick 汇报不完整结果
